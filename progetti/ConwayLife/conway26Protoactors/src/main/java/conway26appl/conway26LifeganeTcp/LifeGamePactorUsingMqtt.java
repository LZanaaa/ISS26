...
public class LifeGamePactorUsingMqtt extends AbstractProtoactor26 {
	...
	protected IApplMessage elabRequest(IApplMessage req) {
		...
		String payload = req.msgContent();

		if (req.msgId().equals("guicmd")) {
			if (payload.startsWith("cell")) {
				String[] coords = payload.replace("cell(", "").replace(")", "").split(",");
				int x = Integer.parseInt(coords[0]);
				int y = Integer.parseInt(coords[1]);
				switchCellState(x, y);
				displayGrid(); // Manda la griglia aggiornata via dispatch

				return CommUtils.buildReply(name, req.msgId(), "cell_ok", req.msgSender());

			} else if (payload.startsWith("clear")) {
				onClear(); // Fa stop, reset e displayGrid

				return CommUtils.buildReply(name, req.msgId(), "clear_ok", req.msgSender());
			}
		}
		...
		CommUtils.outgreen(name + " | default replyMsg to " + replyMsg.msgReceiver());
		return replyMsg;
	}
